...
export const listMine = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const items = await listMineService(req.user.id);
    sendSuccess(res, items);
  } catch (error) {
    sendError(res, 500, { code: 'INTERNAL_SERVER_ERROR', message: 'Internal server error' });
  }
...
export const show = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!item) {
      sendError(res, 404, { code: 'NOT_FOUND', message: 'Item not found' });
      return;
    }
...
export const update = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const item = await updateItem(req.params.id, req.user.id, payload);
